# -*- coding: utf-8 -*-
import datetime, time, types, subprocess
    ...
    if len(sortedList) == 1:
        ...
        for outeritem in iter(sortedList):
            ...
            if t == types.UnicodeType:
                originLineStr = originLine.encode('utf-8')
            else:
                originLineStr = originLine

            tmp_arr = [originLineStr, str(currentAccessTimelong), str(singleAccess), str(firstAccess), str(lastAccess)]
            ...
            '''
            try:
                writelines.append((None, originLine.encode('utf-8') + "\t" + str(currentAccessTimelong) + "\t" + str(
...
if __name__ == '__main__':
    ...
    input = "/input/loghandle/flume_01"
    ...
    # import log_handler from Xxx
    sc = SparkContext(pyFiles=['log_handler.py', 'url_handler.py'], conf=conf)
    sql_context = SQLContext(sc)

    lines = sc.textFile(input)
    parts = lines.map(lambda l: l.split(__separator)).filter(lambda x: len(x) == 17)
    # line --unicode : contain two bytes
